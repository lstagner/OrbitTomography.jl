...
function (S::PolyharmonicSpline)(x::Array{Float64,2})
  m,n = size(x)
...
function (S::PolyharmonicSpline)(x)
    return S(reshape(collect(x),length(x),1))
end

function (S::PolyharmonicSpline)(x,y)
    return S(hcat(x,y))
end

function (S::PolyharmonicSpline)(x,y,z)
    return S(hcat(x,y,z))
end
